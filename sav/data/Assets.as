...
package sav.data
{
	...
	{	
		public static function createSingleton(logStatues:int = 10, masterPath:String = null):Assets
		{
			if (_instance)
				trace('warning : old Assets instance is replaced, resources may not disposed');
			else
				_instance = new Assets(logStatues, masterPath);
			...
			progressFunc:Function = null, progressFuncParams:Array = null):String
		{
			...
			var loadingItem:LoadingItem = _loader.add(url, loaderParams );
		...
		public function loadURL_cancel(loadingItemId:String):void
		{
			var loadingItem:LoadingItem = _loader.get(loadingItemId);
			if (loadingItem)
			{
				_loader.remove(loadingItemId);
		...
		private function singleFileLoaded(evt:Event):void
		{
			...
			if (obj && obj.successFunc) obj.successFunc.apply(null, obj.successFuncParams);
