package sav.game
{
	...
	public class McEffecter
	{
		...
		*********************************************************************************************************************/		
		...
		{
			var x:Number			= (params == null || params.x == undefined) ? 0 : params.x;
			var y:Number			= (params == null || params.y == undefined) ? 0 : params.y;
			var scaleX:Number		= (params == null || params.scaleX == undefined) ? 1 : params.scaleX;
			var scaleY:Number		= (params == null || params.scaleY == undefined) ? 1 : params.scaleY;
			var rotation:Number	= (params == null || params.rotation == undefined) ? 0 : params.rotation;
		...
		private static function effectEnterframeHandler(evt:Event):void
		{
			...
			{
				var p:DisplayObjectContainer = evt.target.parent;
				p.removeChild(DisplayObject(evt.target));
		...
		*********************************************************************************************************************/		
		...
		{
			var object:Object			= {};
			var center:Point			= new Point(targetMc.x,targetMc.y);
		...
		{
			var object:Object;
			for (var i:int=0;i<shakeArray.length;i++)
			{
				object = shakeArray[i];
			...
			var x:Number = Math.sin(Math.PI * object.currentAngle/360) * object.range + object.center.x;
			var y:Number = Math.cos(Math.PI * object.currentAngle/360) * object.range + object.center.y;
			object.targetMc.x = x;
			object.targetMc.y = y;
		...
		*********************************************************************************************************************/
		...
		public static function applyMotion(sourceMc:DisplayObjectContainer , MotionClass:Class , params:Object = null):void
		{
			...
			var removeOnComplete:Boolean		= (params == null || params.removeOnComplete == undefined)	? true : params.removeOnComplete;
			...
			sourceMc.x = 0;
			sourceMc.y = 0;
			var index:int = p.getChildIndex(sourceMc);
		...
		{
			var motion:Object = evt.target;
			if (motion.removeOnComplete)
			{
				var sourceMc:DisplayObject = motion.motionSprite.getChildAt(0);
				removeMotion(sourceMc);				
			}
			if (motion.onCompleteFunction != null) motion.onCompleteFunction();
		}
		
		public static function removeMotion(sourceMc:DisplayObject):void
		{
			for (var i:uint = 0; i < motionArray.length; i++)
			{
				...
				{
					var motion:Object		= object.motion;
					var p:DisplayObjectContainer			= motion.parent;
					sourceMc.x		= motion.x;
					sourceMc.y		= motion.y;
					var index:int	= p.getChildIndex(motion);
