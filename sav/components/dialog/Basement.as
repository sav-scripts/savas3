package sav.components.dialog
{
	...
	public class Basement extends Sprite
	{
		...
		{
			_setting = setting; 
			filters = [new GlowFilter(0, 1, 6, 6, .8, 3)];
		}
		
		internal function resize(bound:Rectangle, btnBound:Rectangle = null, btnRc:Number = 10, scrollerHeight:Number = Number.NaN):void
		{
			_bound = bound;
			...
			var c5:uint = ColorUtils.blend(boardColor, 0x000000, 0.5);
			...
			var c3:uint = ColorUtils.blend(color, 0x000000, 0.18);
			var c4:uint = ColorUtils.blend(color, 0xffffff, 0.08);
			g.beginFill(c3);			
			//GraphicDrawer.drawRoundRect(g, bound, rc);
			
			drawShape(g, bound, btnBound, btnRc, rc, scrollerHeight);
			bound.inflate( -1, -1); rc --; 
			if (btnBound) { btnBound.inflate(1, 0); btnBound.y -= 1, btnRc ++; }
			g.beginFill(c4);
			//GraphicDrawer.drawRoundRect(g, bound, rc);
			...
			var c1:uint = ColorUtils.blend(color, 0x000000, 0.04);
			var c2:uint = ColorUtils.blend(color, 0xffffff, 0.0);
			var c6:uint = ColorUtils.blend(color, 0xffffff, 0.0);
