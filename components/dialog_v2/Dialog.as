package sav.components.dialog_v2
{
	...
	public class Dialog extends Sprite
	{
		 ...
		 * @return	Dialog
		 */
		public static function quickAlert(contentObj:*, buttonArrayOrCloseFunc:* = null, title:String = "MESSAGE", arrangeContent:Boolean = false, setting:DialogSetting = null):Dialog
		{
			var dialog:Dialog = new Dialog();
			
			dialog.toAlert(contentObj, buttonArrayOrCloseFunc, title, arrangeContent, setting);
			dialog.popAt();
			
			return dialog;
		}	
		
		public function toAlert(contentObj:*, buttonArrayOrCloseFunc:* = null, title:String = "MESSAGE", arrangeContent:Boolean = false, setting:DialogSetting = null):void
		{				
		...
		public function update(doRebuildChilds:Boolean = true):void
		{
			...
			{				
				...
				{
					_buttonGroup.build(_buttonData, _bound);
					if (_buttonGroup.width > _bound.width) 
					{
						var oldContentWidth:Number = _bound.width;
						_bound.width = _buttonGroup.width;
						_content.x = int((_bound.width + _setting.contentInflateWidth * 2 - oldContentWidth) / 2);
					}
					
					_buttonGroup.x = _bound.x + (_bound.width - _buttonGroup.width) / 2;
					if (_content.isEmpty == false)
					{
						_buttonGroup.y = _bound.bottom + _setting.contentInflateHeight;					
					}
					else
					{
						_buttonGroup.y = _bound.bottom;
					}
					
					if (_setting.buttonAlign == ButtonAlign.HORIZONTAL)
					{
						_bound.height += _setting.buttonHeight;
					}
					else
					{
						_bound.height += _setting.buttonHeight * _buttonData.length;
					}
					
					_buttonGroup.drawBasement();
				}
				
				_bound.width = Math.max(_bound.width, _basement.titleTF.width);
				
				if (_content.isEmpty == false)
				{
					_bound.inflate(_setting.contentInflateWidth, _setting.contentInflateHeight);
				}
				else
				{
					_bound.inflate(_setting.contentInflateWidth, 0);
				}
			...
			else
			{
				...
				if (_buttonData.length)
				{
					...
					_buttonGroup.x = _bound.x + (_bound.width - _buttonGroup.width)/ 2;
					_buttonGroup.y = _bound.bottom - _setting.buttonHeight;
			...
			if (_buttonData.length)
			{
				...
				var buttonGroupSelfBound:Rectangle = _buttonGroup.getBounds(_buttonGroup);
				_buttonGroup.x = Math.round(_bound.x + (_bound.width - _buttonGroup.width)/ 2 - buttonGroupSelfBound.x);
			}
			else
			{
				btnRc = 0;
			...
			_basement.resize(_title, basementBound, btnBound, _buttonData.length, btnRc, scrollerHeight);
		}
		...
		private function reposition(targetStage:Stage):void
		{
			var bound:Rectangle = this.getBounds(this);
			
			this.x = int((targetStage.stageWidth - this.width) / 2 + _setting.offsetX - bound.x);
			this.y = int((targetStage.stageHeight - this.height) / 2 + _setting.offsetY - bound.y);
